    ...
    if FixPAInc:
        ...
        for aline in log_output:
            if Init:
                ...
                matches = re.search("^PA= (.*) inc= (.*) tanpsi= (.*)\s(.*)$",
                                    aline)
        ...
        if RunMCMC:
            allradspsi_fixincPA = np.arctan(
                tanpsi_fixincPA_MCMC[0]) * 180. / np.pi

            print(">>>fix>>psis_fixincPA ", psis_fixincPA)
            print(">>>fix>>allradspsi_fixincPA", allradspsi_fixincPA,
                  "tanpsi_fixincPA_MCMC[0]", tanpsi_fixincPA_MCMC[0])
            print(">>>fix>>tanpsis_fixincPA_MCMC", tanpsis_fixincPA_MCMC)
            print(">>>fix>>psis_fixincPA_MCMC",
                  180. * (np.arctan(tanpsis_fixincPA_MCMC)) / np.pi)
            ...
            allradspsi_fixincPA = np.arctan(
                allradstanpsi_fixincPA) * 180. / np.pi
            return (RunMCMC, [
                r1s, r2s, rregions_fixincPA, psis_fixincPA,
        ...
        for aline in log_output:
            ...
            if AllRads:
                ...
                if matches:
                    ...
                    matches = re.search("tanpsi-> (.*) $", aline)
                    if (matches):
                        allradstanpsi = float(matches.group(1))
                    else:
                        allradstanpsi = 0.
            ...
            if Regions:
                ...
                matches = re.search(
                    "^PA->\s*(\d+\.\d+)\s*\+\-\s*(\d+\.\d+)\s*inc->\s*(\-?\d+\.\d+)\s*\+\-\s*(\d+\.\d+)\s*tanpsi->\s*(\-?\d+\.\d+)\s*\+\-\s*(\d+\.\d+)\s*$",
                    aline)
                if (matches):
                    aPA = matches.group(1)
