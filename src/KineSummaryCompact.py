...
def addimage(iplotpos,
    ...
    clabels = [
        '%.2f' % (ColorBarScale * clevs[0]),
        '%.2f' % (ColorBarScale * clevs[1])
    ]
    ...
    if (Clevs == 'Region'):  # and (not 'centered.fits' in filename_grey)):
        ...
        scatter_subim = np.sqrt(
            np.median(
                (subim_grey[np.where(subim_region > 0.9)] - mean_subim)**2))
        ...
        if (os.path.exists(filename_weights)):
            ...
            range1 = np.min(
                subim_grey_filt[np.where((subim_region > 0.99)
                                         & (subim_w > (medianval_w)))])
            range2 = np.max(
                subim_grey_filt[np.where((subim_region > 0.99)
                                         & (subim_w > (medianval_w)))])

            scatter_subim_raw = np.std(
                subim_grey[np.where((subim_region > 0.9) & (subim_w > 1E-3))])
            mean_subim = np.median(subim_grey[np.where((subim_region > 0.9)
                                                       & (subim_w > 1E-3))])
            scatter_subim = np.sqrt(
                np.median((subim_grey[np.where((subim_region > 0.9)
                                               & (subim_w > 1E-3))] -
                           mean_subim)**2))

            print("Region + weights: range1, range2", range1, range2)

            print("Region range: ", range1, " ", range2, "soft scatter ",
                  scatter_subim, "hard scatter ", scatter_subim_raw)
            flog.write("Region range: " + str(range1) + " " + str(range2) +
                       " soft scatter " + str(scatter_subim) +
                       " hard scatter " + str(scatter_subim_raw) + "\n")
        ...
        if (SymmetricRange):
            ...
            delta_range0 = max(np.fabs(range1 - av_range0),
                               np.fabs(range2 - av_range0))
            if (UseScatter):
    ...
    if RegionOfInterest:
        ...
        [x_planet, y_planet] = RegionOfInterest
        e = Ellipse(xy=[x_planet, y_planet],
                    width=0.32,
                    height=0.32,
                    angle=0.,
                    color='yellow',
    ...
    if (isinstance(DoGreyCont, str)):
        ...
        print("i0 " + str(i0), "hdr_grey['CRPIX2']", hdr_grey['CRPIX2'])
        levels = [
            -1.65344452058093, -1.6129840879707, -1.55158301988206,
            -1.51002707264227
        ]

        CS = ax.contour(subim_grey_cont,
                        levels,
                        origin='lower',
                        linewidths=1.0,
                        linestyles='solid',
...
def exec_summary_allrads(workdir,
                         filename_source,
                         vsyst=0.,
                         basename_errormap=False,
                         file_m0=False,
                         file_m2=False,
    ...
    if (file_continuum):
        ...
        Resamp.gridding(file_im_continuum,
                        workdir + inbasename + '_centered.fits',
                        fileout=workdir + inbasename + '_subim_continuum.fits')
...
def exec_summary_faceon(workdir,
                        filename_source,
                        vsyst=0.,
                        basename_errormap=False,
                        file_m0=False,
    ...
    if (file_continuum):
        ...
        Resamp.gridding(file_im_continuum,
                        fileref,
                        fileout=workdir + inbasename +
                        '_subim_continuum_faceon.fits')
    ...
    (clevs, clabels) = addimage(iplotpos,
                                ...
                                a_min=a_min,
                                a_max=a_max,
                                FilterOutliers=FilterOutliers)  # ,vsyst=vsyst)
