...
from ImUtils.Resamp import gridding, slice0
...
def exec_conicpolar_expansions(M):
    ...
    for irrs in range(len(rrs)):
        ...
        cosi=np.cos(M.inc)
        Nind=2.*np.pi*thisradius * cosi /(M.bmaj) # aprox number of beams at each radius
        Nsum=len(w_vec)
        Npolcorr=Nsum/Nind
        if Npolcorr < 1:
            Npolcorr=1.
    ...
    if (PlotRadialProfile and DumpAllFitsFiles):
        ...
        plt.ylim(-0.1*np.max(v_Phi_prof),1.1*np.max(v_Phi_prof))
