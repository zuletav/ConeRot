...
class Model():
    ...
    def __init__(
        ...
        for a_attribute in initlocals.keys():
            if VerboseInit:
                print("DConeMaps setting ", a_attribute, " to ",
                      initlocals[a_attribute])
            setattr(self, a_attribute, initlocals[a_attribute])

    def conicpolar_expansions(self):
        return funcs_DConeMaps.exec_conicpolar_expansions(self)

    def prep_files(self):
        funcs_DConeMaps.exec_prep_files(self)

    def grid_4center(self):
        funcs_DConeMaps.exec_grid_4center(self)
