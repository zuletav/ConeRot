...
# This part should be solved to not modify manually
...
class Setup():
    ...
    def Run(self):
        ...
        if self.DoConjGrad:
            ...
            if ((not self.DoErrorMap)
                    and (M.velodev_med > self.typicalerror * 3.)):
                # sys.exit("RERUN with correct typicalerror, recommend to use at least "+str(M.velodev_med))
                print((
                    "RERUN with correct typicalerror, recommend to use at least "
                    + str(M.velodev_med)))
        ...
        if (self.RunMCMC and (os.path.isdir(self.workdir))):
            OptimM = Optim_DCone.OptimModel(M,
                                            ...
                                            n_cores_MCMC=self.n_cores_MCMC,
                                            burn_in=self.burn_in)
            print(
                "MasterDConeMaps: calling OptimM.emcee with self.n_cores_MCMC=",
                self.n_cores_MCMC)
            OptimM.emcee(M)
        ...
        if (self.DoRegions):
            OptimM = Optim_DCone.OptimModel(
                M,
                n_cores_regions=self.n_cores_regions,
                RunMCMC=True,
                Nit=self.Nit,
                nwalkers=self.nwalkers,
                n_cores_MCMC=self.n_cores_MCMC,
    ...
    def RunFixOrient(self,
        ...
        if (not ForceGlobalOrient):
            ...
            print(
                ">>>>>> Doing Fix Orient, with forced global orientation, at PA=",
        ...
        self.tanpsi = tanpsi
        # self.nwalkers = 7 #
        self.domain = (('tanpsi', (tanpsi - self.rangetanpsi / 2.,
                                   tanpsi + self.rangetanpsi / 2.)), )
